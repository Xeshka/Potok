...
interface ChecklistApi {
    ...
    suspend fun getChecklist(@Path("taskId") taskId: String): List<ChecklistTaskDTO>
    ...
    suspend fun toggleChecklistTask(
        @Path("checklistTaskId") checklistTaskId: UUID,
        @Path("done") done: Boolean
    ): ChecklistTaskDTO
    
    @POST("/api/service-task-checklist/api/v1/task-checklist/move")
    suspend fun moveChecklistTask(@Body request: ChecklistTaskMoveRq)
}